...
export function useRegionRandomizer({
  ...
  const trigger2DFadeTransition = useCallback(
    (regionId: string) => {
      const region = regionsRef.current.find((r) => r.id === regionId);
      if (!region || !region.randomizeEnabled) return;
  ...
  const trigger2DZoomTransition = useCallback(
    (regionId: string) => {
      const region = regionsRef.current.find((r) => r.id === regionId);
      if (!region || !region.randomizeEnabled) return;
  ...
  const triggerRandomChange = useCallback(
    ...
    [
      ...
      trigger2DFadeTransition,
      trigger2DZoomTransition,
      getRandomMode2D,
      onUpdateRegion,
    ],
  );
  ...
  useEffect(() => {
    ...
    regions.forEach((region) => {
      ...
      const settingsChanged =
        storedSettings?.interval !== randomizeInterval || storedSettings?.enabled !== shouldBeActive;
  ...
  }, [isVisualizerActive, triggerRandomChange, cancelAnimation, regions]);
